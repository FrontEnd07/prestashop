...
*
*  MODIFIED BY MYPRESTA.EU FOR PRESTASHOP 1.7 AND 8 PURPOSES !
*
*/
...
class myprestacomments extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Product Comments').' '.$this->l('by MyPresta.eu');
    ...
    public function install($keep = true)
    {
        ...
        if (parent::install() == false ||
            !$this->registerHook('displayMyPrestaCommentsFields') ||
    ...
    public function hookdisplayMyPrestaCommentsFields($params)
    {
        return;
    }
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitModerate')) {
            ...
            Configuration::updateValue('PRODUCT_COMMENTS_ONLY_CUSTOMERS', Tools::getValue('PRODUCT_COMMENTS_ONLY_CUSTOMERS', '0'));
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'is_bool' => true,
                        //retro compat 1.5
                        'label' => $this->l('Allow reviews for customers of product only'),
                        'desc' => $this->l('When option is active module will give possibility to place a review about product for customers that placed an order for this specific product. When option will be active - guest reviews will not be available.'),
                        'name' => 'PRODUCT_COMMENTS_ONLY_CUSTOMERS',
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,
    ...
    public function getConfigFieldsValues()
    {
        return array(
            ...
            'PRODUCT_COMMENTS_ONLY_CUSTOMERS' => Tools::getValue('PRODUCT_COMMENTS_ONLY_CUSTOMERS', Configuration::get('PRODUCT_COMMENTS_ONLY_CUSTOMERS')),
        );
    }
    ...
    public function getStandardFieldList()
    {
        ...
        $custom_fields = Hook::exec('actionMyPrestaCommentsFieldBo', array(), null, true);
    ...
    public function checkIfCustomerPurchasedProduct($id_customer, $id_product)
    {
        $orders = Db::getInstance()->ExecuteS('SELECT o.`id_order` FROM `' . _DB_PREFIX_ . 'orders` o INNER JOIN   `' . _DB_PREFIX_ . 'order_detail` od ON o.id_order = od.id_order WHERE o.`id_customer` = ' . (int)$id_customer . ' AND od.`product_id` = ' . (int)$id_product);
    ...
    public function renderCommentsForm($params)
    {
        ...
        if (Configuration::get('PRODUCT_COMMENTS_ONLY_CUSTOMERS') == 1) {
            ...
            if (!$this->checkIfCustomerPurchasedProduct($this->context->customer->id, Tools::getValue('id_product'))) {
                $this->context->smarty->assign('reviewsForCustomersPurchased', 0);
            }
        } else {
            $this->context->smarty->assign('reviewsForCustomers', 1);
